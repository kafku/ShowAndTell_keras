...
im2txt_model.compile(loss="categorical_crossentropy", optimizer=Adam(), metrics=['acc'])
...
checkpoint = ModelCheckpoint(filepath="./results/model_weight/weights_rmsprop64_inceptionv3_{epoch:02d}-{val_loss:.2f}_.hdf5",
                             save_best_only=True)
csv_logger = CSVLogger('./results/logs/show_and_tell_rmsprop64_inceptionv3.csv')
